...
*   **当前版本**: v1.1.0-beta
*   **开发状态**: 已形成可演示主流程，管理员、教师、助教三角色界面与权限已基本分离，但仍处于“功能收口 + 数据真实性补齐 + 风险治理”阶段，不能按生产完成态交接。
...
| **自动化测试** | 🟡 已建立最小集 | 已有后端 `unittest` 和前端 `node:test` 回归 | 当前覆盖权限/管理员界面，未覆盖所有业务链路 |
...
*   教师的数据范围控制尚未完全收口到“只能访问自己课程”，当前更多是角色级限制。
*   文档曾长期滞后于代码，现已开始校正，但后续仍需持续维护。

### 5.4 风险与技术债

*   **权限边界风险**：虽然三角色界面已区分，但教师“仅访问自己课程”的资源所有权控制仍未完全闭环。
...
2.  **优先补安全边界**:
    *   补齐教师仅访问自己课程数据的资源所有权控制。
    *   对管理员用户管理补充更细粒度的保护策略（如禁止误停用当前账号、审计记录等）。

3.  **优先补测试**:
    *   在现有最小回归的基础上，继续补齐登录、学生导入、预警生成、报表导出链路。
