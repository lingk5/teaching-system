...
*   **评分公式**: `综合分 = attendance(20%) + homework(20%) + quiz(20%) + final_exam(30%) + interaction(10%)`
*   **覆盖率规则**:
    *   `0/5` 指标有数据：综合分记 `0`
    *   `1/5` 指标有数据：允许显示综合分，但不生成预警
    *   至少 `2/5` 指标有数据：按覆盖项重算权重，并参与预警判定
*   **预警分级**:
    *   🔴 **红色 (Red)**: < 60分 (高风险)
...
*   支持 **批量导入**：学生名单、考勤记录、平时作业、测验、期末成绩、课堂互动。
*   **健壮性设计**: 
...
| **数据导入** | ✅ 完成 | 学生/考勤/作业/测验/期末/互动导入可用 | 期末成绩已独立建模并接入评分/预警 |
...
| **自动化测试** | ✅ 已建立基线 | 已有后端 `unittest` 和前端 `node:test` 回归 | 已覆盖角色权限、助教指派、覆盖率评分和期末独立链路 |
...
*   仓库已补充回归测试集，当前已覆盖角色权限、助教指派、覆盖率规则和期末成绩独立建模链路。
