...
- **总体阶段**: `Beta / 可演示版本`
- **真实状态**: 已完成“登录 -> 课程/班级 -> 学生 -> 数据导入 -> 预警生成/处理 -> 基础导出”的主闭环，但“学情分析可视化、统计口径统一、安全性加固、测试自动化”仍未完成
...
| 自动化测试 | ❌ 未完成 | 当前仓库没有单元测试/集成测试目录 | 仅完成语法级可编译校验 |
...
| 风险项 | 影响 | 当前表现 |
|--------|------|----------|
| 鉴权边界不一致 | 中高 | `analytics`、`warnings` 的部分查询接口与多个 `data` 写接口未加 `@jwt_required()` |
...
2. 给 `data`、`analytics`、`warnings` 相关接口补齐鉴权和权限边界
3. 补完 `analytics.html` 与 `dashboard.html` 的真实数据渲染，移除模拟图表
4. 为预警引擎增加缓存、增量刷新或定时任务机制
5. 建立最小测试集：认证、导入、预警、导出四条主链路至少各有一组自动化校验
...
### 场景：新增"课堂表现记录"功能（历史示例）

> ⚠️ 说明：本节为“如何扩展新模块”的历史示例模板，不代表当前主干已实现该模块。  
> 当前主干已实现模块请以 `backend/app/routes/` 与 `frontend/src/pages/` 实际文件为准（现有模块：`auth`、`courses`、`data`、`analytics`、`warnings`、`export`）。
...
1. 浏览器访问：`http://127.0.0.1:5000/pages/login.html`
2. 使用演示账号登录：
   - `admin / 123456`
   - `teacher / 123456`
   - `assistant / 123456`
